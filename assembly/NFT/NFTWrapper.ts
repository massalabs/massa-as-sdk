...
export class NFTWrapper {
  ...
  constructor(at: Address) {
    ...
    this._maxSupply = u64(parseInt(call(this._origin, "Symbol", "", 0)));
  ...
  OwnerIndex(address: Address): Array<u64> {
    const arrOfNum: Array<u64> = [0];
    const strArr = call(this._origin, "OwnerIndex", address._value, 0).split(
      ","
    );

    strArr.forEach((str) => {
      arrOfNum.push(u64(parseInt(str)));
    });
    return arrOfNum;
  }
